class Core {
  ...
  constructor () {
    ...
    this.fs.ensureDir(`logs`).catch((err: any) => {
      console.error(err)
    })
    this.localLogger = require('simple-node-logger').createRollingFileLogger(
      this.opts
    )
  ...
  async initialise () {
    ...
    this.log(
      '                               Smartcloudjs framework Launched',
      2
    )
    ...
    if (process.env.DEBUG == 'true')
      this.log(
        'Debug is active, you will get a lot more information than usual',
        1
      )
    if (this.package.version.split('.')[2].split('-')[1]) {
      this.log(
        `Running version development version ${
          this.package.version.split('.')[2].split('-')[1]
        }. Please be careful when running an unstable version`,
        3
      )
      if (process.env.SENTRY_DSN) {
        this.configureSentry(process.env.SENTRY_DSN)
      } else {
        this.log(
          'Sentry not configured. Please use core.configureSentry() from smartcloudJS to setup within your module, refer to our documentation for additional information',
          3
        )
      }
    }
    if (process.env.SENTRY_DSN) {
      ...
      this.log(
        'Sentry not configured. Please use smartcloudJS documentation to setup',
        3
      )
    }
  }
  ...
  configureSentry (
    dsn: string,
    options?: {
      license_email: string | undefined
