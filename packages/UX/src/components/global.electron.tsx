...
export class Container extends React.Component {
  ...
  height = () => {
      this.setState({height: win.getSize()[1], width: win.getSize()[0]});
  };
  ...
  componentDidMount() {
    ...
    win.on("will-resize", debounce(this.height, 200))
    win.on("maximize", this.height)
    win.on("unmaximize", this.height)
    win.on("enter-full-screen", this.height)
    win.on("leave-full-screen", this.height)
    if (Platform.OS !== 'web') {
      this.setState({
  ...
  render() {
      let contain = null
      if (Platform.OS=='web') {contain = {maxHeight:  this.state.height-20, maxWidth:  this.state.width}}
      return (
